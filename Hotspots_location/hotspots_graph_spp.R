...
hotspots_files <- list.files(path = "/Users/bibianaterradasolerdeoliveira/Library/CloudStorage/OneDrive-Personal/Doutorado/Rodosol/Rodosol_data_scripts/Supplementary_material/Hotspot",
                             pattern = "\\.csv$",
                             full.names = TRUE, 
                             all.files = FALSE) 
...
boiy <- rep(13, 14) # boidae
ciy <- rep(12, 13) # coendou  
...
y <- c(boiy, ciy, cty, dasyy, day, hhy, pcy, tty, lly, cpy, eby, smy, sby) #juntando as infos do y das tres espécies num vetor (NOTAR: na mesma ordem que ja aparecem na tabela dos dados)
...
roadkills_km <- read.table(here::here("Hotspots_location","roadkills_km.txt"), h = T) %>%
  filter(specie != "PotentialUsers")
...
Ll <- image_read(here::here("Hotspots_location","silhouette", "ll.png")) %>% image_flop()
...
Eb <- image_read(here::here("Hotspots_location","silhouette", "eb.png")) %>% image_flop()
Cp <- image_read(here::here("Hotspots_location","silhouette", "cp.png")) %>% image_flop()
...
(hotspots_graph <- rodosol + 
    ...
    # CLUSTERS
    annotate("rect", xmin = 43.8, xmax = 46.7, ymin = -1.5, ymax = 14.1,
             fill = "grey80") + #retangulo do cluster km 45
    annotate("rect", xmin = 49.8, xmax = 50.2, ymin = -1.5, ymax = 14.1,
             fill = "grey80") + #retangulo do cluster km 50
    annotate("rect", xmin = 59.3, xmax = 60.2, ymin = -1.5, ymax = 14.1,
             fill = "grey80") + #retangulo do cluster km 59
    geom_richtext(label = "<span style='font-size:9pt; color:black'>Cluster<br>45", x = 45, y = 14.1) +
    geom_richtext(label = "<span style='font-size:9pt; color:black'>Cluster<br>50", x = 50.2, y = 14.1) +
    geom_richtext(label = "<span style='font-size:9pt; color:black'>Cluster<br>59", x = 59.9, y = 14.1) +
    ...
    # city tags
    geom_richtext(label = "<span style='font-size:11pt; color:black'>Vitória", x = 0, y = - 0.5) +
    geom_richtext(label = "<span style='font-size:11pt; color:black'>Guarapari", x = 67.4, y = -0.5) + 
    ...
    annotation_raster(Ci, xmin = 68.7, xmax = 73.3, ymin = 11.8, ymax = 12.3) +
    annotation_raster(Boi, xmin = 69.3, xmax = 73.3, ymin = 12.7, ymax = 13.3))

# save
ggsave(hotspots_graph, filename = here::here("Hotspots_location", "hotspots_graph_spp.png"), 
